...
var {{ visid_raw }}_data = {{ vis_json | safe}};
...
def prepared_data_to_html(data, d3_url=None, ldavis_url=None, ldavis_css_url=None,
    ...
    if visid is None:
        visid_list = list()
        visid_json_list = list()
        for datum in data:
          visid = 'ldavis_' + get_id(datum) + str(int(random.random() * 1E10))
          visid_json_list.append(json.dumps(visid))
          visid_list.append(visid)

    elif re.search('\s', visid):
        raise ValueError("visid must not contain spaces")

    data_json_list = [datum.to_json() for datum in data]
