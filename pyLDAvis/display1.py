...
<script type="text/javascript">
   !function(LDAvis){
       new LDAvis("#" + {{ visid | safe }}, {{ vis_json  }});
   }(LDAvis);
</script>
""")
...
if(typeof(window.LDAvis) !== "undefined"){
   !function(LDAvis){
       new LDAvis("#" + {{ visid | safe  }}, {{ visid_raw }}_data);
   }(LDAvis);
}else{
...
def prepared_data_to_html(data, d3_url=None, ldavis_url=None, ldavis_css_url=None,
    ...
    if visid is None:
        visid = list()
        for datum in data:
          visid.append('ldavis_' + get_id(datum) + str(int(random.random() * 1E10)) )
    elif re.search('\s', visid):
        raise ValueError("visid must not contain spaces")

    vis_json_list = [datum.to_json() for datum in data]

    return template.render(visid=map(json.dumps, visid),
                           visid_raw=visid[0],
                           d3_url=d3_url,
                           ldavis_url=ldavis_url,
                           vis_json=vis_json_list,
