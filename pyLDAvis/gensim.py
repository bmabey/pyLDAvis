...
def prepare(topic_models, corpus, dictionary, doc_topic_dist=None, **kwargs):
    """Transforms the Gensim TopicModel and related corpus and dictionary into
    the data structures needed for the visualization.

    Parameters
    ----------
    topic_models : gensim.models.ldamodel.LdaModel
        An already trained list of Gensim LdaModel. The other gensim model types are
    ...
    **kwargs :
    ...
    prepare_data_list = list()

    for model in topic_models:
      opts = fp.merge(_extract_data(model, corpus, dictionary, doc_topic_dist), kwargs)
      prepare_data_list.append(vis_prepare(**opts))
    
    return prepare_data_list
